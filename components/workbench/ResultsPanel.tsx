...
interface ResultsPanelProps {
    ...
    onSelectFlashcard: (card: WorkbenchFlashcard) => void;
    onAddToNotebook: (card: WorkbenchFlashcard) => void;
}

const FLASHCARDS_PER_PAGE = 5;

export const ResultsPanel: React.FC<ResultsPanelProps> = ({ flashcards, onSelectFlashcard, onAddToNotebook }) => {
        ...
        <>
            ...
            <div className="overflow-y-auto p-3 space-y-3 flex-grow">
                {paginatedFlashcards.length > 0 ? paginatedFlashcards.map(card => (
                    <div key={card.id} onDoubleClick={() => onAddToNotebook(card)} className="bg-slate-800/70 p-3 rounded-lg shadow-md hover:bg-slate-800 border border-transparent hover:border-slate-700 transition-all cursor-pointer">
                        <div className="flex justify-between items-start mb-2">
                            ...
                            <div className="flex items-center gap-1">
                                <button onClick={() => onSelectFlashcard(card)} className="p-1.5 rounded-md hover:bg-slate-700" title="Ver documento fuente">
                                    <EyeIcon className="w-4 h-4 text-gray-400"/>
                                </button>
                                <button onClick={() => onAddToNotebook(card)} className="p-1.5 rounded-md hover:bg-slate-700" title="Agregar al cuaderno">
